...
// IsRequestScoped is deliberately false: an upstream stream that disconnects
// before response.completed is a connection/upstream failure, not a client
// request problem. Keeping it unscoped lets the conductor rotate to another
// credential instead of returning the error to the caller.
