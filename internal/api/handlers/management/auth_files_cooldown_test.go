...
func TestListAuthFiles_ModelLevel403CooldownExpired(t *testing.T) {
	...
	// The model-level cooldown has elapsed, but 403 is a terminal credential failure in this fork
	// (see isCredentialAuthFailureError), so the auth must stay in error state instead of being
	// reconciled back to active. Only the expired retry deadline is cleared.
	registerAuthForLookupTest(t, manager, &coreauth.Auth{
	...
	file := payload.Files[0]
	// 403 is treated as a terminal credential failure: the elapsed cooldown must not
	// reconcile this auth back to active.
	if !file.Unavailable {
		t.Error("expected unavailable=true for 403 credential failure, got false")
	}
	if file.Status != string(coreauth.StatusError) {
		t.Errorf("expected status=%q, got %q", coreauth.StatusError, file.Status)
	}
	if file.StatusMessage != "forbidden" {
		t.Errorf("expected status_message=%q, got %q", "forbidden", file.StatusMessage)
	}
	if !file.NextRetryAfter.IsZero() {
		t.Errorf("expected zero next_retry_after, got %v", file.NextRetryAfter)
	}
