...
func isAuthBlockedForModel(auth *Auth, model string, now time.Time) (bool, blockReason, time.Time) {
	...
	// xAI quotas are account-scoped, so an active model quota can block
	// other models on the same credential. Codex model failures remain scoped
	// to the observed model unless explicitly marked credential-scoped.
