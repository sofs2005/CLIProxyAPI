...
func isAuthBlockedForModel(auth *Auth, model string, now time.Time) (bool, blockReason, time.Time) {
	...
	// different model alias. For non-account-scoped providers, a matching model
	// state is authoritative so an expired model cooldown can admit the credential
	// even before the aggregated auth-level fields are refreshed.
	provider := strings.ToLower(strings.TrimSpace(auth.Provider))
	accountScopedQuota := provider == "xai" || provider == "codex"
	if model == "" || accountScopedQuota || !hasModelStateForModel(auth, model) {
...
func hasModelStateForModel(auth *Auth, model string) bool {
