...
// the start of the ring. This resumes the rotation at the credential that follows the
// previous pick even when candidates were filtered out in between. An empty lastID starts
// at the head.
func successorIndex(available []*Auth, lastID string) int {
	if lastID == "" {
		return 0
	}
	// Exact match keeps rotation correct for candidate lists that are not sorted by ID
	// (e.g. Codex credentials ordered by nearest quota reset).
	for i, auth := range available {
		if auth != nil && auth.ID == lastID {
			return (i + 1) % len(available)
		}
	}
	// Candidates sorted by ID resume via binary search, including filtered-out picks.
...
func isAuthBlockedForModel(auth *Auth, model string, now time.Time) (bool, blockReason, time.Time) {
	...
	// xAI and Codex quotas are account-scoped, so any active model quota blocks the
	// whole credential. Other providers keep model-specific quota isolation.
	provider := strings.ToLower(strings.TrimSpace(auth.Provider))
	accountScopedQuota := provider == "xai" || provider == "codex"
	if model == "" || accountScopedQuota {
