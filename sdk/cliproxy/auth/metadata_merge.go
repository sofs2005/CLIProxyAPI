...
func MergeRefreshedAuth(base, current, updated *Auth) *Auth {
	...
	if finalDisabled {
		...
		// A successful token refresh recovers credential failures (401/403 and plain
		// runtime errors), so their error/cooldown state must not be carried over onto
		// the refreshed auth. Quota/rate-limit (429) and transient upstream (5xx)
		// failures are not recoverable by refresh and stay preserved below.
		credentialFailureRecovered := base != nil && !hasNonCredentialFailure(base) &&
			(base.Status == StatusError || base.LastError != nil || base.Unavailable)
		if hasNewConcurrentError && !credentialFailureRecovered {
		...
		} else if !credentialFailureRecovered && current.Quota.Exceeded && current.Quota.Reason == "credential_quota" && current.Quota.NextRecoverAt.After(time.Now()) {
			// Preserve active credential quota
			merged.Unavailable = current.Unavailable
			merged.Status = current.Status
			merged.StatusMessage = current.StatusMessage
		} else if !credentialFailureRecovered && current.Unavailable && current.NextRetryAfter.After(time.Now()) {
