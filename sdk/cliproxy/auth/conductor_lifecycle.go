...
func (m *Manager) updateInternal(ctx context.Context, base, auth *Auth, mode updateAuthMode) (*Auth, error) {
	...
	if !existing.Disabled && existing.Status != StatusDisabled && !auth.Disabled && auth.Status != StatusDisabled {
		...
		// Refresh results already resolved their own failure state against base
		// (MergeRefreshedAuth), so re-applying it here would resurrect the very
		// error the refresh just cleared.
		if mode != updateModeRefresh {
